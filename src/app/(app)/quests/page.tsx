...
function QuestList({ list }: { list: TeamQuest[] }) {
  if (list.length === 0) return <p className="text-sm text-ssyu-brown/40">배정된 퀘스트가 없습니다.</p>;
  return (
      ...
      {list.map((tq) => (
        <div key={tq.id} className={`rounded-2xl p-4 shadow-sm ${tq.completed ? "bg-ssyu-mint/20" : "bg-white"}`}>
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-bold text-ssyu-brown/40">{tq.progress}/{tq.quest.targetCount}</span>
          </div>
          <p className="text-xs text-ssyu-brown/50 mt-1">{tq.quest.description}</p>
...
export default function QuestsPage() {
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className="font-display text-xl">퀘스트</h1>
      <section>
        <h2 className="font-display text-[15px] mb-2">일일 퀘스트</h2>
        <QuestList list={daily} />
      </section>
      <section>
        <h2 className="font-display text-[15px] mb-2">주간 퀘스트</h2>
