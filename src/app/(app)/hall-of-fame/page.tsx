  ...
  return (
    <div className="p-4 space-y-4">
      <div className="text-center space-y-1 py-4">
        ...
        <h1 className="font-display text-xl">명예의 전당</h1>
        <p className="text-sm text-ssyu-brown/50">모두가 함께 키운 쓔들을 한자리에 모았습니다</p>
      </div>
      <div className="grid grid-cols-2 gap-4">
        {rows.map((r) => (
          <div key={r.teamId} className="rounded-3xl bg-gradient-to-b from-white to-ssyu-yellow/20 shadow p-3 flex flex-col items-center">
            ...
            <div className="text-[11px] text-ssyu-brown/40">{r.region} · {r.xp} XP</div>
          </div>
        ))}
      </div>
    </div>
